...
;             
;sta_apid: the STATIC data product to use, as a string. Choose from 'ce', 'cf', 'd0', 'd1'. Default is 'd0' if not set.   
;
; 
;             
;OUTPUTS:
...
;mvn_sta_flow, /flux, mrange=[12., 20.], m_int=16., sta_apid='d1', spicekernels=kk  ;calculate flow vectors for all available timestamps. SPICE is loaded before hand.
;
;
;timespan, '2019-02-03', 1.
;mvn_sta_l2_load, sta_apid=['d1'], /tplot_vars_create
;ctime, tt   ;for now, this routine requires a time range input as flow calculations take a long time on an entire day.
;mvn_sta_flow, /flux, mrange=[12., 20.], m_int=16., sta_apid='d1', trange=tt  ;calculate vectors for selected time range only. SPICE is loaded within the routine.
;
;
...
if size(sta_apid,/type) eq 0 then sta_apid = 'd0'
...
if flux eq 1 then begin
    tname = 'mvn_sta_flow_flux_m'+massSTR
    get_4dt, t1=tr[0], t2=tr[1], 'j_4d','mvn_sta_get_'+sta_apid,mass=mrange, name=tname,energy=erange,bins=bins;,m_int=m_int    
    ...
    if sc_vel eq 1 then begin
      ...
      tname_den = 'mvn_sta_flow_dentmp'+massSTR
      get_4dt, t1=tr[0], t2=tr[1], 'n_4d', 'mvn_sta_get_'+sta_apid, mass=mrange, name=tname_den, energy=erange, bins=bins;, m_int=m_int
    endif
endif

;FLOW IN VEL: needed for units of km/s
if kms eq 1 then begin
     tname = 'mvn_sta_flow_kms_m'+massSTR
     get_4dt, t1=tr[0], t2=tr[1], 'v_4d', 'mvn_sta_get_'+sta_apid, mass=mrange, name=tname, energy=erange, bins=bins;, m_int=m_int 
...
;FLUX:
if flux eq 1 then begin
    tname = 'mvn_sta_flow_flux_m'+massSTR  ;get correct tplot variable
    ...
    if sc eq 1 then begin
        ...
        fname_flux = 'mvn_sta_flow_flux_m'+massSTR+'_MSO'
        store_Data, fname_flux, data={x: times, y: rotateARR_full}
          ...
          options, fname_flux, ytitle='STA flow!C'+massSTR+'!C[#/cm2/s]'
...
;VEL:
if kms eq 1 then begin
    tname = 'mvn_sta_flow_kms_m'+massSTR  ;get correct tplot variable
    get_data, tname, data=dd2
    ...
    if sc eq 1 then begin
        ...
        fname_kms = 'mvn_sta_flow_kms_m'+massSTR+'_MSO'               
        store_data, fname_kms, data={x: times, y: rotateARR_full}  ;can't do log as can have -ve values. Leave as log for now.
          ...
          options, fname_kms, ytitle='STA flow!C'+massSTR+'!C[km/s]'
...
if sc_vel eq 1 then begin
    ...
    if kms eq 1 then begin
        ...
        store_data, fname_kms2, data=ddTMP2, dlimit=dlTMP, limit=llTMP
          options, fname_kms2, ytitle='STA flow!C'+massSTR+'!Csc-corr!C[km/s]'
        ...
        store_data, fname_kms3, data=ddVEL, dlimit=dlTMP, limit=llTMP
          options, fname_kms3, ytitle='STA flow!C'+massSTR+'!Csc corr factor!C[km/s]'
    ...
    if flux eq 1 then begin 
        ...
        store_data, fname_flux2, data=ddFLUX2, dlimit=dlTMP, limit=llTMP
          options, fname_flux2, ytitle='STA flow!C'+massSTR+'!Csc-corr!C[#/cm2/s]'
        
        fname_flux3 = fname_flux+'_sc_correction_factor'
        store_data, fname_flux3, data=corr_factor, dlimit=dlTMP, limit=llTMP
          options, fname_flux3, ytitle='STA flow!C'+massSTR+'!Csc corr factor!C[#/cm2/s]'
